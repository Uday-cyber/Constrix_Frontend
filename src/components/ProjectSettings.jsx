    ...
    return (
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className={cardClasses}>
                <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-300 mb-4">{t("taskDetails.projectDetails")}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Name */}
                    <div className="space-y-2">
                        <label className={labelClasses}>{t("createProject.projectName")}</label>
                        <input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className={inputClasses} required />
                    </div>

                    {/* Description */}
                    <div className="space-y-2">
                        <label className={labelClasses}>{t("createProject.description")}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className={labelClasses}>{t("createProject.status")}</label>
                        ...
                        <div className="space-y-2">
                            <label className={labelClasses}>{t("createProject.priority")}</label>
                    ...
                    <div className="space-y-4 grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className={labelClasses}>{t("createProject.startDate")}</label>
                        ...
                        <div className="space-y-2">
                            <label className={labelClasses}>{t("createProject.endDate")}</label>
                    ...
                    {/* Progress */}
                    <div className="space-y-2">
                        <label className={labelClasses}>{t("taskDetails.progress")}: {formData.progress}%</label>
                    ...
                    <button type="submit" disabled={isSubmitting} className="ml-auto flex items-center text-sm justify-center gap-2 bg-gradient-to-br from-blue-500 to-blue-600 text-white px-4 py-2 rounded" >
                        <Save className="size-4" /> {isSubmitting ? t("profile.saving") : t("projectSettings.saveChanges")}
                    </button>
                </form>
            </div>
                ...
                <div className={cardClasses}>
                    <div className="flex items-center justify-between gap-4">
                        <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-300 mb-4">
                            {t("createProject.teamMembers")} <span className="text-sm text-zinc-600 dark:text-zinc-400">({project.members.length})</span>
                        </h2>
                        ...
                        <div className="space-y-2 mt-2 max-h-32 overflow-y-auto">
                            {project.members.map((member, index) => (
                                <div key={index} className="flex items-center justify-between px-3 py-2 rounded dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-300" >
                                    <span> {member?.user?.email || t("team.unknownUser")} </span>
